...
<body
	...
	<div class="app-body">
		...
		<main class="main">
			<center>
				<a href="Buyer_CheckCollectionListAction?pageNo=1">首页</a>
				<c:if test="${pageNo>1}">
					<a href="Buyer_CheckCollectionListAction?pageNo=${pageNo-1}">上一页</a>
				</c:if>

				<a href="Buyer_CheckCollectionListAction?pageNo=${pageNo+1}">下一页</a>
			</center>
			<div id="BookMain">


				<c:forEach var="stu" items="${Buyer_CheckCollectionList}">
					<div class="Book001">
						...
						<p align="center">
							...
							<br> 类型:
							<c:out value="${stu.favType}" />
							<br> 买家姓名:
							<c:out value="${stu.uname}" />
						</p>
						<a href="Buyer_CheckBookAction?BId=${stu.BId}">查看详情</a> <a
							href="buyer_DelBookCollectionAction?FId=${stu.FId}"
							style="float:right;">移出收藏夹</a>
					</div>
				</c:forEach>
			</div>
		</main>
