...
<body
	...
	<div class="app-body">
		...
		<main class="main">
			<center>
				<a href="buyer_CheckCarAction?pageNo=1">首页</a>
				<c:if test="${pageNo>1}">
					<a href="buyer_CheckCarAction?pageNo=${pageNo-1}">上一页</a>
				</c:if>

				<a href="buyer_CheckCarAction?pageNo=${pageNo+1}">下一页</a>
			</center>

			<c:forEach var="stu" items="${buyer_Car_List}">
				<table width="100%">
					<tr>
						...
						<th>
							<p>
								数量：<a href="Buyer_Car_Add?pageNo=1&CId=${stu.CId}">
									<button>+</button>
								</a> <span><c:out value="${stu.bnumber}" /></span> <a
									href="Buyer_Car_Plus?pageNo=1&CId=${stu.CId}"><button>-</button></a>


							</p>
							<p>
								应付价钱：
								<c:out value="${stu.shouldpay}" />
							</p>
						</th>
						<th>
							...
							<p>
								<a
									href="Buyer_AddpayAction?BId=${stu.TBook.BId}&TCount=${stu.bnumber}&setmeals=${stu.package_}&shouldPay=${stu.shouldpay}">
									购买</a>
							</p>
							<p>
								<a href="Buyer_DelCar_Action?CId=${stu.CId}"> 移出购物车</a>
							</p>
						</th>
					</tr>
				</table>
