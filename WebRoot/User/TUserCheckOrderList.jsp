	...
	<div class="app-body">
		...
		<main class="main">
			<center>
				...
				<a href="Buyer_CheckOrderListAction?pageNo=1">首页</a>
				<c:if test="${pageNo>1}">
					<a href="Buyer_CheckOrderListAction?pageNo=${pageNo-1}">上一页</a>
				</c:if>

				<a href="Buyer_CheckOrderListAction?pageNo=${pageNo+1}">下一页</a>
			</center>
			<c:forEach var="stu" items="${buyer_CheckOrderList}">
				<table width="100%">
					<tr>
						<th><img width="200px" height="200px" alt="图片"
							src="files/${stu[0].bpicture}"></th>

						<th>
							<p>
								买家姓名：
								<c:out value="${stu[0].uname}" />
							</p>
							<p>
								套餐详情：
								<c:out value="${stu[0].package_}" />
							</p>
							<p>
								卖家姓名：
								<c:out value="${stu[0].sname}" />
							</p>
						</th>
						...
						<th>
							<p>
								数量：
								<c:out value="${stu[0].tcount}" />
							</p>
							<p>
								订单状态：
								<c:out value="${stu[0].ostate}" />
							</p>
							<p>
								发货状态：
								<c:out value="${stu[0].logistics}" />
							</p>
						</th>
						<th>
							<P>
						...
						<c:choose>
							<c:when test="${stu[0].logistics=='等待卖家发货'}">
								<th>
									<p>
							...
							<c:when
								test="${stu[0].ostate=='已完成' and stu[0].logistics=='已签收'}">
								<th>
									<p>
										<a href="TUserEVtran?OId=${stu[0].OId}&bname=${stu[1]}">评价</a>
									</p>
								</th>
							</c:when>
							<c:when
								test="${stu[0].ostate=='已完成' and stu[0].logistics=='买家已评价'}">
								<th>
									<p>
										<a href="">订单已完成</a>
									</p>
							...
							<c:when
								test="${stu[0].logistics=='卖家已发货' and stu[0].ostate=='未完成'}">
								<th>
									<P>
										<a href="buyer_SignAction?OId=${stu[0].OId}" onclick="fun1()">签收</a>
									</P>
								</th>
							</c:when>
						</c:choose>
