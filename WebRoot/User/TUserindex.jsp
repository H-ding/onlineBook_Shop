...
<body
	...
	<div class="app-body">
		<div class="sidebar">
			<nav class="sidebar-nav">
				<ul class="nav">
					...
					<!-- 升级为VIP-->
			<li class="nav-item"><a class="nav-link"
				href="buyer_BecomeVip?<%=request.getSession().getAttribute("username")%> ">
					<i class="icon-calculator"></i> 成为VIP <span
					class="badge badge-info"></span></a></li>
					
					<li class="divider"></li>
					<li class="nav-title">退出系统</li>
					<li class="nav-item nav-dropdown"><a
						class="nav-link nav-dropdown-toggle" href="#" onClick="custom_close()"><i
							class="icon-star"></i> exit</a></li>
		...
		<main class="main">
			...
			<div id="BookMain">
					<c:forEach var="stu" items="${Buyer_list}">
						<div class="Book001">
							<img height="200px" width="100%" alt="图片"
								src="files/${stu.bpicture}">
								<p align="center">
								书名:<c:out value="${stu.bname}" /> <br>
								 作者:<c:out value="${stu.bauthor}" /> <br>
								  价格:<c:out value="${stu.bprice}" />
								</p>
							<a href="Buyer_CheckBookAction?BId=${stu.BId}">查看详情</a> <a
							href="Buyer_AddCollectionAction?bid=${stu.BId}"
